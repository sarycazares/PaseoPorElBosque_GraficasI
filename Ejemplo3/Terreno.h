...
public:
	...
	Terreno(HWND hWnd, OpenGLClass* OpenGL, const wchar_t* alturas, const wchar_t* textura, const wchar_t* textura2,
		const wchar_t* textura3, const wchar_t* textura4, const wchar_t* blendingmap,
		float ancho, float prof, int numtext, int numtext2, int numtext3, int numtext4, int numtex5)
	{
		anchof = ancho;
		proff = prof;
		//cargamos la textura de la figura
		Carga(alturas);
		//en caso del puntero de la imagen sea nulo se brica esta opcion
		terreno = Plano(Ancho(), Alto(), ancho, prof, Dir_Imagen(), 1);
		deltax = anchof / Ancho();
		deltaz = proff / Alto();
		verx = Ancho();
		verz = Alto();
		...
		Carga(blendingmap);
		// Set the unique texture unit in which to store the data.
		OpenGL->glActiveTexture(GL_TEXTURE0 + numtex5);
